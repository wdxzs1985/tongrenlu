            ...
            <c:if test="${!empty page.items}">
                ...
                <ul class="pagination visible-xs visible-sm clearfix">
                    <c:forEach begin="1" end="${page.pagenum}" varStatus="status">
                        ...
                        <a href="<c:url value="/user/${userBean.userId}/fans?page=${status.count}"/>"><c:out value="${status.count}"/></a>
                        </li>
                    </c:forEach>
               </ul>
...
$(function() {
    ...
    $("#jpaginate").paginate({
        ...
        onChange                : function (currval){
            window.location.href = '<c:url value="/user/${userBean.userId}/fans"/>?page=' + currval;
            return false;
        }
    });
});
