...
<html>
<head>
<title><fmt:message key="app.name"/> > 标签：<c:out value="${tagBean.tag}"/> > 音乐</title>
        ...
        <div class="row">
            <div class="col-md-8 col-lg-8">
                <div class="page-header">
                    <h4>与<a href="<c:url value="/tag/${tagBean.tagId}"/>"><c:out value="${tagBean.tag}"/></a>相关的音乐</h4>
                </div>
                    ...
                    <c:forEach items="${page.items}" var="articleBean" varStatus="status">
                        <div class="media">
                            <a href="<c:url value="/music/${articleBean.articleId}"/>" 
                                ...
                                <img class="" src="<%= FILE_PATH %>/${articleBean.articleId}/cover_120.jpg">
                            </a>
                            <div class="media-body">
                                ...
                                <p class=""><c:out value="${articleBean.description}"/></p>
                            </div>
                        </div>
                    </c:forEach>
                </div>
                <ul class="pagination visible-xs visible-sm clearfix">
                    <c:forEach begin="1" end="${page.pagenum}" varStatus="status">
                        <li class="<c:if test="${page.page eq status.count}">active</c:if>">
                            <a href="<c:url value="/tag/${tagBean.tagId}/music?page=${status.count}"/>"><c:out value="${status.count}"/></a>
                        </li>
                    </c:forEach>
                </ul>
...
$(function() {
    ...
    $("#jpaginate").paginate({
        ...
        onChange                : function (currval){
            window.location.href = '<c:url value="/tag/${tagBean.tagId}/music"/>?page=' + currval;
            return false;
        }
    });
});
