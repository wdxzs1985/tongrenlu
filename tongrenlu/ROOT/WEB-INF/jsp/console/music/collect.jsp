            ...
            <div class="col-md-8 col-lg-9">
                ...
                <ul class="pagination visible-xs visible-sm clearfix">
                    ...
                    <li class="<c:if test="${paginateSupport.page eq status.count}">active</c:if>">
                        <a href="<c:url value="/console/music/collect?page=${status.count}"/>">
...
$(function() {
    $("#jpaginate").paginate({
        ...
        onChange                : function (currval){
            window.location.href = '<c:url value="/console/music/collect"/>?page=' + currval;
            return false;
        }
    });
    $("a[role='delete']").click(function(){
        if(confirm('真的要删除这个投稿吗？')){
            return true;
        }
        return false;
    });
